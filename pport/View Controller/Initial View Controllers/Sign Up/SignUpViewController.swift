...
class SignUpViewController: InitialBaseViewController {
    ...
    @IBAction func signUpBtnTapped(_ sender: Any) {
        ...
        APIManager.shared.singUp(name: name, surname: surname, email: email, password: password, onFailure: { title, body in
            DispatchQueue.main.async {
                self.showPopUp(title: title, body: body)
            }
        }, onSuccess: { data in
            DispatchQueue.main.async {
                self.showPopUp(title: "Thank you", body: "Account succesfully registered")
            }
        })
    }
}
