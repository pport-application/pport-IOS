...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        if let _ = UserDefaultsManager.shared.get(forKey: "name"),
           ...
           let _ = UserDefaultsManager.shared.get(forKey: "email") {
            ...
            let mainTabBarController = storyboard.instantiateViewController(withIdentifier: "NavigationMenuBaseController") as! NavigationMenuBaseController
    ...
    func returnWelcomePageViewController() {
        guard let window = self.window else {
            return
        }
        UserDefaultsManager.shared.removeAll()
